...
@section('contenu')
    ...
    <div class="max-w-4xl mx-auto p-5 article-container">
        ...
        @if($article->image)
            <div class="my-8 text-center">
                @if(str_contains($article->image, 'images/'))
                    <img src="{{ asset($article->image) }}" alt="Image de l'article" class="max-w-full h-auto rounded-lg shadow-lg shadow-primary/20 border border-primary/30">
                @else
                    <img src="{{ asset('storage/' . $article->image) }}" alt="Image de l'article" class="max-w-full h-auto rounded-lg shadow-lg shadow-primary/20 border border-primary/30">
                @endif
            </div>
        @endif
        ...
        {{-- Caractéristiques --}}
        <section class="mb-8">
            <h2 class="text-2xl font-bold text-primary mb-4 pb-2 border-b-2 border-primary">🏷️ Caractéristiques</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-green-500/20 border border-green-500/30 rounded-lg p-4">
                    <h3 class="font-bold text-green-400 mb-2">⏱️ Rythme</h3>
                    @if($article->rythme)
